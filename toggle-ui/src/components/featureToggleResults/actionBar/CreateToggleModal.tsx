export const CreateToggleModal = ({
    flagId,
    setCreatedFlagId,
}: {
    flagId: string;
    setCreatedFlagId: (id: string) => void;
}) => {
    return (
        <dialog
            className={`${
                flagId === "" ? "hidden" : ""
            } z-50 fixed top-0 left-0 right-0 bottom-0 flex items-center w-screen h-screen p-4 bg-gray-800 bg-opacity-80 overflow-y-auto`}
        >
            <div className="max-w-xl w-full mx-auto bg-gray-500 rounded-xl overflow-hidden">
                <div className="max-w-sm mx-auto pt-12 pb-8 px-5 text-center">
                    ...
                    <h4 className="text-xl text-gray-100 font-semibold mb-5">
                        Please save the Toggle ID now, it will disappear after
                        closing modal!
                    </h4>
                    <p className="text-gray-300 font-medium">{`${flagId}`}</p>
                </div>
                <form
                    method="dialog"
                    className="pt-5 pb-6 px-6 text-right bg-gray-600 -mb-2"
                >
                    <button
